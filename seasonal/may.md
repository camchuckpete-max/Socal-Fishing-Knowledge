---
type: seasonal
tags: [spring, bluefin, yellowtail, paddies, flying-fish, size-class, run-and-gun]
sources: [S2L3KLSQ6Is]
confidence: medium
regions: [socal-bight]
waters: [bay-harbor, nearshore-coast, island, bank, open-ocean]
layout: v2
regime: warming
---

# May

**Regime: [warming transition](../conditions/water-regimes.md)** — typical-year
mapping; in an anomalous year trust the live water, not the calendar.

**These are priors — the pattern layer, not current intel.** They weight zones
before you pull live [BightSST](../planning/day-plan-protocol.md) conditions.
A stale report is not today's water.

## Bluefin creep into US waters

- **[Bluefin](../species/bluefin-tuna.md) creep into US waters in May, ahead of the
  volume.** The leading edge arrives before the main body of fish — the
  fishery opens up as they push north (`S2L3KLSQ6Is`).
- **First [paddy yellowtail](../species/yellowtail.md) appear with them.** The first
  [kelp paddy](../conditions/kelp-paddies.md) yellows of the year show up on the same
  push — start checking paddies (`S2L3KLSQ6Is`).

## How to fish the May bluefin

- **Night bite plus daytime flying-fish imitations for grade.** The night bite
  is active, and the bigger fish come to surface presentations that mimic
  flying fish during the day (`S2L3KLSQ6Is`).
- **Run-and-gun casting opens for small boats.** May is when the fishery
  becomes reachable and castable from a skiff — surface fish you can run to and
  throw at, versus April's deep sonar-school game (`S2L3KLSQ6Is`). See
  [foamer casting](../techniques/foamer-casting.md).

## Read the size class — it changes the presentation

Bluefin group by size class, and **seeing the 80+ class changes how you
present** (`S2L3KLSQ6Is`) — ⚠ Fact-check (single-source): breakpoints verified
verbatim against the cited transcript but corroborated nowhere else in the
note's citations; numerically precise and load-bearing for this section.

| Size class | Read |
|---|---|
| **≤ 60 lb** | school-grade fish |
| **60–80 lb** | mid grade |
| **80–100+ lb** | the jumbos — when you see this class present, change your presentation to target them |

Identify the class you are looking at before you commit gear. See
[bluefin-tuna](../species/bluefin-tuna.md) for the presentation-by-grade logic and
[surface iron](../techniques/surface-iron.md) for casting to grade.

Read the water with [water temperature](../conditions/water-temperature.md),
[water color](../conditions/water-color.md), and [current structure](../conditions/current-structure.md).


<!-- backlinks:start -->
## Linked from

- [Bluefin Tuna](../species/bluefin-tuna.md)
- [California Barracuda](../species/barracuda.md)
- [Kelp Paddies](../conditions/kelp-paddies.md)
- [Pacific Bonito](../species/bonito.md)
- [Southern California Bight](../locations/socal-bight.md)
- [Water Regimes](../conditions/water-regimes.md)
- [Yellowtail](../species/yellowtail.md)
<!-- backlinks:end -->
