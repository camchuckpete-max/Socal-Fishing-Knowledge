---
type: jurisdiction
tags: [paperwork, regulations, mexico, fmm, conapesca, insurance, baja]
sources: [P36VGPPf120, odiIWmut6gM, cameron]
confidence: medium
layout: v2
authority: INM (entry) + CONAPESCA (fishing)
as_of: 2026-08
---

# Mexican waters

Everything south of the border needs paperwork a US angler does not carry by
default, and the checks are real rather than theoretical. This page is the
paperwork rung of the geographic ladder: what to buy, what to print, and what
to have to hand. It covers every Mexican region the KB holds — `baja-pacific-north`,
`baja-pacific-south`, `cortez-north` and `cortez-south`
([regions & waters](regions.md); region pages land with the geo phase) — and it
says nothing about how to fish. That is the regions and zones below it.

**Regulatory — Mexico. Jurisdiction: INM for entry, CONAPESCA for fishing.
Checked as of 2026-08. Verify current requirements before you travel: these
change, and this is a foreign jurisdiction.**

## Papers you need

1. **FMM — the tourist permit.** Filled out online in advance; needs passport
   details, point of entry and length of stay, and produces a printout.
   **Print it.** One angler reported being checked three times on a single
   trip (`P36VGPPf120`).
2. **Mexican fishing licence (CONAPESCA).** Bought online. The licence moved to
   a *new* website as of 2026, so search fresh rather than trusting an old
   bookmark (`P36VGPPf120`). Carry your passport when you buy.
3. **Passport.**
4. **Vehicle insurance — separate, and mandatory in practice.** US policies do
   not carry over. **Baja Bound** is the named recommendation; a 4-day trip ran
   about **$90** for a 2021 car and about **$130** for a newer Tacoma
   (`P36VGPPf120`).

⚠ Flagged gap — no corpus source: whether a boat run from San Diego into
Mexican water for the day — the Coronado Islands case —
requires an FMM at all, and what a private boat as opposed to a vehicle must
carry. This is the single most common Mexican-water trip in the KB and the
corpus does not answer it.

⚠ Flagged gap — no corpus source: the Midriff-islands biosphere bracelet, and
US declaration forms for bringing fish home.

## On the water

⚠ Flagged gap — no corpus source: boarding and inspection practice, what the
Mexican Navy asks for, and how paperwork should be presented.

Rules that belong to one place rather than to Mexican water in general live on
that zone's page — a closure or restricted area is a fact about the island, not
about the jurisdiction.

## Bringing fish home

⚠ Flagged gap — no corpus source: US customs declaration for fish caught in
Mexico, transport and limits at the border.

Species and bag limits set by Mexican law are regulations, not fishery
knowledge, and are recorded per-species with their jurisdiction and as-of date;
see [regulatory claims](../sources/regulatory-claims.md). A Baja limit that
differs from a CDFW limit is two jurisdictions, never a doctrine conflict.

## Regions

Four regions sit on this side of the line: the Pacific coast of Baja, split
north and south at the BC/BCS state line, and the Sea of Cortez, split the same
way ([regions](regions.md)). The paperwork on this page applies to all four;
how they fish does not — a Cortez day and a Pacific-coast day share only the
permit.

<!-- children:start -->
- [Baja Pacific — North](baja-pacific-north.md)
- [Baja Pacific — South](baja-pacific-south.md)
- [Cortez North](cortez-north.md)
- [Cortez South](cortez-south.md)
<!-- children:end -->

## Evidence

Trip reports and per-source provenance:
[evidence file](evidence/mexican-waters.md).

<!-- backlinks:start -->
## Linked from

- [1010 Trench / 378 / 213](1010-trench-378-213.md)
- [372 / 245 / 250](372-245-250.md)
- [385 / 238 / 475](385-238-475.md)
- [Bahía de los Ángeles (BOLA / Bay of LA)](bahia-de-los-angeles.md)
- [Baja Pacific — North](baja-pacific-north.md)
- [Baja Pacific — South](baja-pacific-south.md)
- [Cedros / San Benitos](cedros-island.md)
- [Colonet](colonet.md)
- [Coronado Islands](coronado-islands.md)
- [Coronados - 230 / 302 (Kidney Bank) / 226](coronados-230-302-226.md)
- [Cortez North](cortez-north.md)
- [Cortez South](cortez-south.md)
- [Ensenada](ensenada.md)
- [Evidence — Mexican waters](evidence/mexican-waters.md)
- [Finger Bank rockfish](finger-bank-rockfish.md)
- [Imperial Beach](imperial-beach.md)
- [International Artificial Reef](international-artificial-reef.md)
- [La Fonda / Bajamar / Salsipuedes](la-fonda-bajamar-salsipuedes.md)
- [Loreto & Puerto Escondido](loreto.md)
- [Magdalena Bay (Mag Bay / Lopez Mateos)](bahia-magdalena-lopez-mateos.md)
- [Pukey Point](pukey-point.md)
- [Punta Banda / Santo Tomas](punta-banda-santo-tomas.md)
- [Rosarito / Descanso](rosarito-descanso.md)
- [San Quintin](san-quintin.md)
- [The Pistol / Bell Bank (300)](pistol-bell-bank-300.md)
- [Upper Cross / 421 / 390](upper-cross-421-390.md)
- [US waters](us-waters.md)
- [W. Butterfly / 157](w-butterfly-157.md)
<!-- backlinks:end -->
