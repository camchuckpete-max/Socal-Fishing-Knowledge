...
tags: [spotted-bay-bass, spotties, a-rig, umbrella-rig, alabama-rig, swimbaits, eelgrass, mission-bay, bays, harbors]
sources: [G6YRT4HNxr8, 6-mi3Qxn37c]
confidence: high
regions: [socal-bight]
waters: [bay-harbor]
layout: v2
...
**One species, one technique.** The umbrella rig is a wire "spider" harness — a
...
fish is a spotted bay bass over a grass flat.

## When this is the call

**Covering water when you want multiple hookup chances at once.** This is the
row the router ranks for putting a small school in front of a fish rather than a
single bait — see
[spotted bay bass — situations → techniques](spotted-bay-bass.md#situations--techniques).

**Open eelgrass, with the canopy setting the depth.** The rig is worked over
grass flats and kept just above the canopy so it stays in the strike zone
(`6-mi3Qxn37c`); why the grass holds fish and how to read its edge is in
...
arm for a keel effect (`6-mi3Qxn37c`).

**A bay bass hits the rig from behind**, so slack has to come out of the line
fast — the stated reason for the high gear ratio in the build below
(`6-mi3Qxn37c`).

## Rigs & gear

- **Harness:** a wire "spider" A-rig (Recon Baits style) carrying several small
  swim jigs, built to an **anchovy profile** (`G6YRT4HNxr8`).
- **Arm bend:** bend the short arms **outward** for an even spread while the
  longer centre arm(s) stay centred, using a **parabolic (gradual) bend, not a
  hard or sharp one** — a sharp bend point-loads and weakens the wire
  (`G6YRT4HNxr8`).
- **Hook count: five** — one per jig, four arms plus one centre
  (`G6YRT4HNxr8`, `6-mi3Qxn37c`). Five hooks is fine in saltwater; a personal
  **three-hook cap** applies instead in freshwater. No jurisdiction or as-of
  date is given for that freshwater cap, so treat it as a stated personal
  operating rule rather than verified regulation text, and verify current CDFW
  freshwater and saltwater gear rules before relying on it (`G6YRT4HNxr8`).
- **Knot: a Palomar tied over the whole rig**, rather than a uni knot — a uni
...
- **Line: 60–80 lb braid to a 40 lb fluorocarbon top shot**, run mainly for
  ...
  fluorocarbon** on the rig's dropper/leader material — against anglers who fish
  as light as **6 lb** test for spotties around pilings (`6-mi3Qxn37c`).
- **Reel: a low-profile 200-size at 7.3:1 with about 20 lb of max drag** — the
  fast ratio is valued for taking up slack when a bass hits from behind and for
  pitching to dock pilings (`6-mi3Qxn37c`).
- **Rod:** a heavy cranking / A-rig blank (`6-mi3Qxn37c`).
- **Class term:** the heavier end of the
  [bass casting / swimbait class](../tackle/gear-classes.md), which is the class
  line that already carries the double-rig and A-rig braid/fluoro step-up.
- **Baits:** small paddle-tails on leadheads —
  [bay bass plastics](../lures/bay-bass-plastics.md),
  [soft-plastic swimbaits](../lures/soft-plastic-swimbaits.md).

The same harness fished for barred sand bass is a different presentation
entirely — sink it and cruise the baits back up through the column — in
...
- **Five baits on a wire harness, not two off a swivel.** The general note's
  multi-bait material is the two-swimbait double rig on a three-way swivel with
  12 in and 24 in leaders, aimed at the followers trailing a hooked kelp calico;
  this is a five-jig harness fished as its own presentation over a grass flat
  (`G6YRT4HNxr8`).
- **The build decisions move from the hook to the hardware.** The general note's
  rigging is a keel-weighted hook and a bait-butting spring; here the decisions
  are arm bend geometry, hook count, head weight distribution and the knot tied
  over the whole rig (`G6YRT4HNxr8`).
...
[evidence file](evidence/spotted-bay-bass-swimbaits.md). Full spotted bay bass
source provenance:
