...
The signature SoCal inshore ambush predator: a kelp- and reef-dweller that sits
on a **defined structure edge** and eats what the **current** sweeps past it.
This note routes the fishery — where, when, how to find them, which technique per
situation, and what gear class. Execution lives in the linked technique notes;
structure detail lives in [bass structure](../locations/bass-structure.md).
Primary voice is Erik Landesfeind (Academy Ep. 5, Ep. 13, Ep. 14, dated reports).

The load-bearing contrast to keep straight: **calico are current-driven and
largely tide-indifferent** — the opposite of the harbor
[spotted bay bass](spotted-bay-bass.md), which are fully tide-driven. For calico
you read the *current across the kelp*, not the tide clock.

## Where & when

- **Where:** kelp beds, reef, boiler rocks, breakwalls/jetties, and island
  structure from the coast out to Catalina, Clemente, and the Channel Islands.
  Fish the **edges and pinch-points** — leading edge, outside edge, isolated
  stringers off the main bed — not the dense middle
...
- **Current, not calendar/tide:** the parallel-current rule governs. Current
  ...
  gets skipped ([current structure](../conditions/current-structure.md)).
- **Water:** track **water temperature and color spot to spot** (Ep. 14) — you
...
- **Observed** (StokedOnFishing, 2014-11-03 upload, Bird Rock, Catalina Island,
  ...
  confidence): an anchored spot near Bird Rock produced 3–4 lb calico bass
  alongside yellowtail (confirmed on live squid) and barracuda from the same
  stop — see the parallel entry in [yellowtail](yellowtail.md).
...
- **Observed** (StokedOnFishing, 2015-01-18, Cedros Island, Baja —
  ...
  bite once the fish were located — roughly **10–15 calico an hour**
  sustained, spiking to **"30 an hour at one point"** in a patch of
  distinctly "super blue" water — described on camera as fish "chewing
...
- **Observed** (StokedOnFishing, 2014-11-13, Geronimo Island / Chester's
  ...
  `_C8w6zeVPak` "Shogun Skiff Trip part 3" entries elsewhere in this KB —
  evidently a separate year's trip reusing the same numbering): an
  afternoon session off the 90-ft mothership Shogun produced a
  fast, high-volume calico bite on **surface iron and swimbaits**,
  described on camera as "endless miles" of calico water, with multiple
  doubles landed working the ground from Geronimo Island to Chester's Rock.
  The same ground was mixed-species enough that calico repeatedly
  interrupted a deliberate yellowtail push the following morning — "pesky
  5-pound calicoes... putting off my 50-pound yellow" — see the
...
- **Observed** (StokedOnFishing, 2017-04-07 upload, West End of Cedros
  ...
  Unregistered channel — medium confidence.
- **Observed** (StokedOnFishing, 2019-03-12 upload, West End of Cedros
  Island ("Jurassic Park") backside, Baja — `3yK3JYrKoZY`, "Big Yellowtail,
  Cedros Island Part 2"; distinct trip/date from, and corroborating, the
  `RPSRH0jwyw4` West End/"Jurassic Park" entry above — same nickname for the
  same ground): the crew again worked the backside/West End, noting on
  camera it was **one of only two or three boats fishing the backside** that
  day and calling it a rarely-fished "window." Weedless swimbaits produced a
  fast bite with repeated doubles (one hookup called a "double banger") and
  multiple solid calico landed and released; the crew is also heard targeting
  California halibut nearby with a swim bait, though the audio at that point
  is too garbled to confirm whether a fish was actually landed, or its
  weight/depth (see [California Halibut](california-halibut.md)). The
  episode's closing recap describes fishing conditions immediately preceding
  this trip (exact date not given) as having included a **hurricane swell and
  wind**, yet the island still produced quality yellowtail and calico around
...
- **Observed** (StokedOnFishing, 2018-02-01 upload, La Bocana, Baja California
  ...
  destination reached via a co-op-run fishing village, called out on camera as
  giving traveling anglers "no crowds, no other boats" because the local
  co-op controls access to protect the fishery and the community. Working
  rock structure a few miles from the beach camp on plastics and surface
  iron produced a wide mixed bag across the trip (yellowtail — see the
  [yellowtail](yellowtail.md) Baja coast entry, sierra (as named on camera),
  [barracuda](barracuda.md), calico bass, grouper, needlefish, sand bass, and
  [bonito](bonito.md) all named on camera), with the calico bite grading up
  through one session — repeated **5 lb** fish "one after another," then a
  fish that came to the boat hoped to be the "magical" double-digit 10 lb
...
- **The edge is the spot.** On any kelp head, the biting fish sit on the
  **up-current / leading edge** — "hit three or four sides of it and you know in
  two or three casts" (Ep. 14). Calico face **into the current** and ambush from
  the front of the kelp, so a lure swept past the leading edge draws them out
  ([Benny Florentino](https://www.youtube.com/watch?v=Rwy4MqeXCIU)).
- **Read the current off the kelp:** kelp **pulled 20 ft under** = raging current
  (surface presentations dead); **laid down** = strong; **resurfacing** =
  slacking, switch presentation back ([current diagnostics](../conditions/current-diagnostics.md)).
- **Birds:** **cormorants diving *in* the kelp** = actively feeding = a calico
  tell, better than gulls inshore; a mixed pile *on* kelp/structure can be a bait
  event worth a look. Gulls locked to a kelp edge with nothing better around are
  worth checking ([bird reading](../conditions/bird-reading.md)).
- **Sonar signature:** calico mark **tight to the kelp and structure edges and on
  hard bottom** — fish sitting on the reef-to-sand transition and the outside
  kelp line rather than roaming open water. Typical working depths run the
  **outside kelp edge ~50 ft in to ~25 ft**, and calico will rise **10–15 ft off
  80–100 ft of reef** to eat a crankbait or sunk iron. A solid unbroken wall of
...
prime spot they're usually not in the bed. Beds re-bite, so come back later
(Ep. 13). Boat positioning wins — the console jogs the boat so the bow angler can
present the outside stringers.

## Gear summary (class terms)

- **Rod/reel:** 7–9 ft **graphite-composite bass class** (200–300 size reel,
  30–65 lb braid) for slugs/weedless/hard baits; a **jig-stick class** for
  surface iron; a **crankbait class** (8–10 ft, 200 reel) for cranking.
...
- **Size-DOWN to match small anchovy bait (Tackle Express, `SclIN_ceduA`,
  2026-07-10; unregistered channel, single mention — medium confidence):** a
  different, condition-specific size call than the trophy-grade bullet
  above — when small anchovies are the bait present, downsize to match them
  rather than fishing a bulky plastic, because "the fish get really keyed in
  on it." Demonstrated on a small **Z-Man Diesel Minnow** (~1/4 oz lead
  head) fished off a Kicker-brand Alabama rig; see [A-Rig (Alabama
  Rig)](../lures/a-rig.md). Kept side by side, not reconciled — trophy-grade
  sizes up deliberately for bigger fish/fewer bites, this sizes down to match
  present forage; both are size-matching calls made under different stated
  goals.
...
- **Parallel-current exception (kept side by side):** late fall at San Clemente
  ...
  Encode parallel as the prior, into-beach as a viable late-fall/off-color state,
  off-beach as dead ([current structure](../conditions/current-structure.md)).
...
- **Baja regional note — Cedros Island voluntary catch-and-release (as reported
  ...
  **region label, not a doctrine conflict** with the CDFW 14 in SoCal size
  limit above — two different jurisdictions. **Same interview also aired in
  an earlier upload** (StokedOnFishing, `Zo92MG459gQ`, "Did I Not Say There
  Are Giant Yellowtail In Here," 2023-05-24 upload — predating the
  `ldVj0BoB-kE` 2023-10-16 upload above): matching Jose dialogue giving the
  same rationale for the same three species, confirming this is one
  interview re-cut across at least two videos, not independent
  confirmation — the underlying interview itself was recorded on or before
  2023-05-24, not October 2023 as the later upload date alone would
  suggest.

- **Observed (Baja, Ensenada, Pacific side — region label)** (StokedOnFishing,
  2026-02-17 upload, a point/reef a few miles down the coast from Ensenada,
  Baja California — `0dIwWiOc1NY`, "What It's Like Fishing in Ensenada,
  Mexico"; unregistered channel — medium confidence): a single calico bass
  came incidentally while the crew slow-trolled mackerel and cast jigs for
  yellowtail on a reef/point stop — see the
  [trolling](../techniques/trolling.md#ensenada-baja--slow-trolled-mackerel-plus-cast-jigs-for-yellowtail-on-a-reefpoint)
  Ensenada entry for the technique. The crew called it "one of the best
  eating fish in the ocean" on camera — a table-quality opinion, not
  technique doctrine, logged here alongside rather than folded into the
  existing catch-and-release ethic entries above (this fish was kept, not
  released).
...
- **Observed** (StokedOnFishing w/ Capt. Benny Florentino, San Clemente
  ...
  this specific point — medium confidence.
- **Observed** (StokedOnFishing, 2024-11-13, Long Beach breakwater, SoCal —
  `usHl-4SfqDA`; unregistered channel — medium confidence): a SoCal-specific
  (not Baja) statement of the same voluntary catch-and-release ethic already
  logged above for Cedros/Baja — the crew names the local slogan "our bass
