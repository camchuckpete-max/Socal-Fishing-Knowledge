...
A connection knot purpose-built to join **monofilament to fluorocarbon** —
...
same-name/different-mechanic conflict stays visible rather than getting
buried in a larger knots catalog.

## When to use it

- Tying a **fluorocarbon leader/bite-guard onto a monofilament main line** —
  the flyline application named across every source below.
...
- **A fifth tie, this time from SearcherSportfishing rather than
  StokedOnFishing, and a turn-count data point for the same loop/figure-eight
  family (Rick Maxa, Fisherman's Landing Tackle, via SearcherSportfishing,
  "Tackle Tip Thursday Vol. 212 (Connection Series: Seaguar Knot),"
  `M-W5mEjh1MY`, 2023-01-19; unregistered channel/speaker — `medium`
  confidence):** cut a **3–4 ft** piece of fluorocarbon for the leader/bite
  guard, then oppose it to the monofilament main line coming off the reel
  (lay the two lines running opposite directions from each other, matching
  Cesar's "opposite directions" lay in `48ZFXnCTTQE` above, though the
  fold/wrap mechanic that follows matches the loop/figure-eight family below
  rather than Cesar's two-and-a-half-turn tie). Take the two opposed lines and
  form a loop, held between thumb and finger; take **three turns** in that
  loop ("finger inside, one, two, three turns"). Work a hand back through the
  loop's opening, capture the tag along with the whole fluorocarbon length in
  that hand, and draw the bundle out through the loop — before cinching, the
  result should show a "little figure eight" shape, the same tell named in
  the `tzeXXPAjqUY` and
  `i3qIAHW-SJc` entries above ("if you got that figure eight, you've tied it
  right"). Grab all **four strands** (two main lines, two tags) and pull them
  together slowly; just before the knot fully seats, wet it, drop the two tag
  ends, pull the two main lines together the rest of the way, then trim.
  **Reads as a fourth data point in the loop/wrap-back/figure-eight family**
  (`06lxuie5cZQ`, `tzeXXPAjqUY`, `i3qIAHW-SJc` above) rather than Cesar's
  opposite-direction two-and-a-half-turn tie, and the first of that family to
  state an unambiguous, non-ASR-flagged turn count: **3 turns** — consistent
  with (not necessarily identical to) the "three times, four times"
  ASR-uncertain count in `tzeXXPAjqUY` and the "one two three" ASR-uncertain
  count in `i3qIAHW-SJc`, but stated cleanly here rather than garbled. Also
  gives a leader-length figure (**3–4 ft**), in the same range as (though not
  identical to) Cesar's **~3 ft** fluoro-tag figure above — the only other
  entry in this note with a stated length. **What the transcript doesn't
  resolve:** a
  79-second clip with thin auto-generated captions and no line class,
  species, or on-the-water application named; the opening lines describing
  which piece is "the main line" versus the freshly-cut fluorocarbon are
  slightly garbled in the transcript, resolved here against the presenter's
