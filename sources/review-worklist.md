...
| locations/north-point.md | gazetteer | transformed | gaps(5) | new spot page under cedros-island.md from the single cite wdbqTio1SQU; all 4 skeleton sections plus a 2-row species table (yellowtail, everything-else gap); no coordinates published (name-only ground, no spot-lists entry); the bait-stop-then-run day shape and the yo-yo hookup kept, and the trip's opening surface-iron fish explicitly placed on the arrival-afternoon session rather than this point; added to the parent zone child list |
