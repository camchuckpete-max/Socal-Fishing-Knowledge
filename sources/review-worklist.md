...
| bait/making-bait.md | standard | transformed | misplaced(1: queued to relocation-queue.md), spots(8), asr-uncertain(3) | bait/evidence/making-bait.md / migrated to layout v2: source-named headings dissolved into content sections, 5 Observed blocks split to evidence with mechanism trace kept beside doctrine; verifier restored a dropped sabiki definition, fixed 2 mislabeled harvest rows, and repaired a cross-note anchor broken by the heading rename (bait/bait-tanks.md); 291->213 lines note, +60 evidence |
