...
| locations/isla-carmen-north-end.md | gazetteer | transformed | gaps:5 fc:0 drift-fixed:6 | apply-with-changes — new spot page under loreto.md from the single cite n4nMCLLW3tY (a named destination, ~20 miles/~1 hour from Puerto Escondido, never reached — the run cancelled by a sheared shift-linkage pin); 4 skeleton sections plus a 1-row species table (all-species gap); no coordinates published (spot library holds nothing on this coast); linked from the parent zone child list and Spots list; verifier corrected six fabricated/mis-cited claims (invented trip sequence in three places, 'statute miles' unit, 18 ft Grady White mis-cite, Honeymoon Bay island placement) |
