...
| locations/isla-monserrat-north-end.md | gazetteer | transformed | gaps:7 fc:2 stub:1 | new spot page under loreto.md from the single cite n4nMCLLW3tY (the 2023-07-31 SD Fish and Sips run out past Six Mile Reef); 4 skeleton sections plus a 4-row species table (cabrilla, roosterfish, houndfish, everything-else gap); no coordinates published (the spot library carries nothing on this coast); 7 gaps, 1 roosterfish-router stub, 2 fact-check rows (both Standing) — the north end is named only in the plan and the local word, with every later call to the island whole, and the north-end-is-best ranking is relayed hearsay with no other end of the island ever fished on camera; parent child… |
