...
| locations/isla-espiritu-santo.md | gazetteer | transformed | gaps(7) | new spot page under la-paz.md from the single cite 92y14x33etQ (the trip's last day); 4 skeleton sections plus a 3-row species table (pargo/mullet snapper, cabrilla, everything-else gap); no coordinates published (no spot-lists entry anywhere on the Cortez coast); written as the zone's middle depth rung — island rock fished close in as a bait-and-bottom stop, abandoned mid-morning on a no-current read before the El Bajo run; line-class and program numbers left attributed to the zone page rather than pulled onto the island; added to the parent zone child list |
