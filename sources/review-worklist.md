...
| locations/cedros-gono-islands-kelp-bed.md | gazetteer | skipped |  | skipped: not a spot — an unnamed kelp bed anchored on off the mother ship, and 'Gono' is an ASR-uncertain island name (the harvest row itself marks it 'asr-uncertain name'); no public/charted identity, absent from sources/spot-lists.md, no coordinates available in-repo; its single claim (bombed casts let fall at anchor over kelp, bit on the fall, `NGxyOlPx3ug`) is already carried by locations/cedros-island.md (Structure & bathymetry, kelp paragraph) and techniques/surface-iron.md |
