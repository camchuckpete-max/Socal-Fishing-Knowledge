...
- **locations/bahia-de-los-angeles.md**
  ...
  - `What's there`: gap — season | Seen randomly and hard to time. The source had not caught one and describes them as migrating on conditions and bait, present both here and higher up in the Sea of Cortez — "you kind of just got to get lucky" (`P36VGPPf120`) ⚠ Fact-check (contradicted-internal): the row routes roosterfish to [Pacific crevalle jack](../species/pacific-crevalle-jack.md), but that note is *Caranx caninus* and its own sourced quotes list roosterfish **beside** jacks as a separate fish; `P36VGPPf120` says "rooster fish", never crevalle or toro. Roosterfish has no router in the KB, so the arrow may be conflating two species |
...
- **locations/magdalena-bay-estuary-channel.md**
  ...
  - `What's there`: gap — season | Named by the guide as inshore targets going into the day, none shown caught in this water (`isXJONlpUP4`). ⚠ Fact-check (contradicted-by-source): wrong speaker and one sharpened ID. The halibut/bay-bass/grouper list is the dockside lodge contact's answer in the morning interview — "halibut spotted babe ass / hopefully hopefully snook / i know there's pargos and snapper / maybe maybe a grouper" (@00:02:18) — and it says "halibut", never **California** halibut. Enrique the panga guide gives his own, shorter list on the run: "okay let's bring knapper grouper / snook" (@00:03:04), with no halibut and no bay bass. Grouper are a boca and mangrove-root fish everywhere else in the corpus ([zone page](bahia-magdalena-lopez-mateos.md#whats-there)) |
...
- **locations/san-diego-artificial-reefs.md**
  ...
  - `What's there`: gap — season shape scoped to this zone | Named in the artificial-reef species mix (`bM2vUS1B-yQ`) |
  - `What's there`: gap — season shape scoped to this zone | Named in the artificial-reef species mix (`bM2vUS1B-yQ`); big white seabass come off chummed reefs once a frenzy is going (`f4qYtHACGyk`) |
  - `What's there`: gap — season shape scoped to this zone | Yellowtail have been taken off an artificial reef on the anchored chum method (`f4qYtHACGyk`) — the outside case, not the expectation ⚠ Fact-check (contradicted-by-source): the downgrade is not in the source, which lists yellowtail in the same breath as the rest with no hedge — "i've got yellowtail using this exact same method", alongside "how many big sheep head i've caught using this method" (`f4qYtHACGyk`). |
  - `What's there`: gap — season | Sargo to **5–6 lb** off chummed artificial reefs (`f4qYtHACGyk`), and named in the reef species mix (`bM2vUS1B-yQ`). ⚠ Fact-check (contradicted-by-source): the figure is stated as what the species can reach, not as reef catches — "those sargos can get up to five and six pounds they're really awesome fish" (`f4qYtHACGyk`) — so "off chummed artificial reefs" attaches a size range to a catch context the source does not give it. ⚠ Flagged stub — no corpus source yet for a sargo species router |
  - `How it fishes`: zone guide missing
  - `How it fishes`: zone guide missing
  - `How it fishes`: gap — the zone's own start-and-move program
