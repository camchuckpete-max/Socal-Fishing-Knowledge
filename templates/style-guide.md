...
| `⚠ Fact-check (<category>): <one line>` | Fact-check finding; categories: `single-source`, `contradicted-by-source`, `contradicted-internal`, `external-mismatch`, `unverifiable`, `cite-unresolved`. A category may carry a **disposition qualifier** — `(cite-unresolved, standing)`, `(contradicted-internal, resolved by geometry)` — recording that the question was answered without deleting the flag it was answered about. Every flag also gets a row in `sources/fact-check-ledger.md`. **Flags never delete or reword the claim they sit beside** — single-source does not mean wrong; Cameron adjudicates the ledger. A ledger row MAY also stand without an inline flag when it records a **demotion adjudication** (a claim moved to the evidence file under the single-observation rule) — the evidence entry cross-references the row. |
| `⚠ cite-unresolved: <original>` | A legacy cite that could not be mechanically resolved to a source id. |
| `⚠ adjudicated (Cameron, <date>)` | Cameron has personally ruled on this passage. It is FINAL: the transform pass preserves its substance and its `(cameron)` attribution verbatim, never rewords it toward the wording it replaced, never re-flags it, never demotes it as single-source, and never "reconciles" it with a source that disagrees — the ruling IS the reconciliation (`prompts/review-note.md` rule 3a). The guard conserves the `cameron` cite token; this marker protects the wording. |
