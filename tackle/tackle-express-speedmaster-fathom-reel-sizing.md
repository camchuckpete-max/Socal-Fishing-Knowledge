...
off mid-sentence before the third tier's line/leader spec is given.

Split into its own note rather than added to [rod & reel
selection](rod-and-reel-selection.md) (already well over this repo's
~400-line size guideline) or [gear classes](gear-classes.md) — linked from
both.
...
- **Small tier — Speedmaster 10 / Penn Fathom 25:** spooled **50–60 lb
  ...
  fishing reels."
- **Mid tier — Speedmaster 16 / a Penn Fathom model captioned "Sporty":** the
  "Sporty" caption doesn't match any recognized Penn Fathom product name, so
  per the ASR-hazard rule the model name itself isn't carried as a specific
  product ID here — flagged `asr-uncertain`; the size-tier position (one step
  up from the small tier, one step down from the big tier) is kept. Spooled
  **80–100 lb braid** to a **40 lb–to–60/80 lb test leader** (the captions
  render the top end of this range ambiguously as "60 to 80," so both the
  60 lb and 80 lb readings are preserved rather than picking one — flagged
  `asr-uncertain`). No named application beyond the general step-up in grade
  implied by the heavier line.
...
- No species-specific decision logic, technique mechanics, or rigging is
  given — this is a size-to-line-class comparison only, consistent with the
  `parameter-skim` depth this video was assigned.
...
- Two other Tackle Express clips share the Speedmaster-vs-Fathom theme
  (`h3ZM-mnSNJ8`, an intro-only teaser with no parameters, and `K_0AeM4OvuE`)
  but are separate, still-pending videos in this corpus, not yet extracted —
  not treated as corroboration of the figures above.

## Related

- [Rod & reel selection](rod-and-reel-selection.md) — general
  application-based reel sizing this ladder sits alongside.
- [Gear classes](gear-classes.md) — the class-term lexicon this ladder's
  tiers map onto loosely (small tier ≈ workhorse live-bait class; big tier ≈
