...
1. Pull SST per spot and along the route (observed and forecast).
2. Find the **breaks** (sharp SST gradients), not just the warmest cell —
   sharp offshore edges can jump multiple degrees in ~1 mi.
3. For each target species, pick the **side** of the break per the rules above,
   and combine with [water color](water-color.md) — never SST alone.

## Related

- [Water color](water-color.md) — the second axis; chlorophyll age and clarity.
- [Current structure](current-structure.md) — what builds and holds the breaks.
- [Upwelling and turnover](upwelling-and-turnover.md)
- [October](../seasonal/october.md) — the regime-change month where temp reading
  matters most.
