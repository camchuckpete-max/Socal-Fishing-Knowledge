---
type: conditions
tags: [birds, terns, shearwaters, gulls, pelicans, cormorants, bird-radar, search]
sources: [Blh2BA-7Ono, OYOda6T3f-8, HzE4FHHcvTk, yLpDI8jnizU, cameron]
confidence: high
---

# Bird Reading

Birds are the fastest long-range fish detector on the water. This is a **merge
note**: the BD corpus doctrine (Erik Landesfeind's dated reports, Duane Diego
Mellor) and **Cameron's own bird model** (source `cameron`, memory 8/3/26) are
kept **side by side, attributed** — the corpus fills exactly the gaps Cameron
says his model is missing. Use with
[search and glassing](../planning/search-and-glassing.md) for the glassing
protocol and effective ranges.

## The core question for every bird

Not "are there birds?" but **"are they on bait, or on bait being pushed by fish
underneath?"** Read *behavior*, not just presence.

## Terns — the best bird (both voices agree)

- **Corpus (7/21/22):** the best bird. **Locked-in repeated dives on one spot =
  bait only.** A **moving dive-line = bait being pushed by fish underneath** —
  that is the one to run on.
- **Cameron:** his best bird too. He will almost always stop and cast on working
  terns, and **offshore he'll follow terns around to find fish even with no other
  birds working.** His tern read is strong; he is actively building the same
  read for the other species below — which is where the corpus doctrine slots in.

## Shearwaters — the gap Cameron's model wants filled

Cameron **doesn't factor shearwaters in (as of 2026-08).** The corpus carries a full
shearwater doctrine (7/21/22) — merge it in as the missing piece:

- **Rafted (sitting in a group)** = sitting over deep bait, waiting for the tide.
- **Drive-through test:** circle straight back = fish under them; scatter and land
  elsewhere = bait only.
- **Rafts hopscotching 20–30 ft at a time** = tracking deep fish.
- **Racing off low in one direction from a boil** = they run to the **head** of
  the school — follow them; the old boil is the tail.

## Gulls

- **Corpus:** unreliable — "parking-lot birds." Only meaningful when **locked in
  at islands / kelp edges** with nothing better around.
- **Cameron:** "good sign but dumb" — a general measure of life in an area, and
  his **paddy-finders** (see [kelp paddies](kelp-paddies.md)). Same read from
  both: gulls tell you there's life and floating kelp, not that gamefish are
  under them.

## Pelicans

- **Corpus:** worthless as a gamefish indicator.
- **Cameron:** "dirty birds" — diving pelicans usually mean he's in the **wrong
  area**; casting there typically produces **mackerel**. (Which makes them a
  useful *bait* tell — where to go catch mackerel, not gamefish.)

## Cormorants

- **Corpus:** **diving *in kelp* = actively feeding = a bass indicator**, better
  than gulls inshore.
- **Cameron:** previously wrote them off as bait stealers; **open to using them
  as a bait-depth indicator.** The corpus read (in-kelp cormorants = feeding =
  bass below) is the concrete version of that instinct.

## Boobies (Cameron)

- Mostly seen at the **Coronado Islands**; Cameron considers them **worth paying
  attention to.** No corpus doctrine to merge here — carried as Cameron's
  observation.

## Mixed piles and birds on the water

- **Mixed piles** (gulls + pelicans + terns + cormorants) **at open-water current
  breaks = bait only — skip** (corpus). **Exception:** a mixed pile **on
  kelp/structure** can be a real bait event worth a look — e.g. the grunion blitz
  that produced every-cast bass in ugly water (10/20/22).
- **Birds sitting still on the water still mark the zone.** Trolling past sitting
  birds and hopscotching rafts produced immediate bites when nothing was flying
  (8/17/22). Riprap version: birds sitting on the rocks mark the thumb-over-hose
  deviation.

## Bird radar (Duane)

- Gain near max, **range 4–5 mi**; bird schools paint as **red blotches**,
  sometimes boat-sized (Duane's fish-finder walkthrough). That **4–5 mi** is the
  working detection envelope to expect from a T-top bird-mode radar.

## Cameron's field notes

- **Observed** (cameron, Aug 2026, east of the 9 Mile Bank SD): probable
  **black-footed albatross** — unusually close to shore.

## Quick reference

| Bird | Behavior that means *fish* | Behavior that means *bait / skip* |
|---|---|---|
| **Terns** | moving dive-line | locked dives on one spot |
| **Shearwaters** | circle back on drive-through; hopscotching rafts; racing low off a boil | rafted and scatter-off; no movement |
| **Gulls** | locked in at island/kelp edge (weakly) | anywhere else — "parking-lot" / paddy-finder |
| **Pelicans** | — (worthless) | diving = wrong area = mackerel water |
| **Cormorants** | diving *in kelp* = bass below | elsewhere |
| **Mixed piles** | on kelp/structure = possible bait event | open-water break = bait only, skip |


<!-- backlinks:start -->
## Linked from

- [August](../seasonal/august.md)
- [Bluefin Tuna](../species/bluefin-tuna.md)
- [Break Walls, Jetties and Riprap](../locations/breakwalls-jetties-riprap.md)
- [Calico Bass (Kelp Bass)](../species/calico-bass.md)
- [California Barracuda](../species/barracuda.md)
- [Dorado (Mahi-Mahi)](../species/dorado.md)
- [Fishing Live Bait](../bait/fishing-live-bait.md)
- [Kelp Paddies](kelp-paddies.md)
- [October](../seasonal/october.md)
- [Pacific Bonito](../species/bonito.md)
- [Pacific Crevalle Jack (Toro)](../species/pacific-crevalle-jack.md)
- [Search and Glassing](../planning/search-and-glassing.md)
- [Skipjack Tuna](../species/skipjack-tuna.md)
- [Spotted Bay Bass (Spotties)](../species/spotted-bay-bass.md)
- [Striped Marlin](../species/striped-marlin.md)
- [Yellowfin Tuna](../species/yellowfin-tuna.md)
- [Yellowtail](../species/yellowtail.md)
<!-- backlinks:end -->
