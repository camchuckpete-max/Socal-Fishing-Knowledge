---
type: planning
tags: [regulations, limits, cdfw, conapesca, california, mexico, jurisdiction, on-demand]
sources: [cameron]
confidence: high
---

# Regulations — California and Mexico

The knowledgebase's one regulatory reference. It exists so limits live in a
single place a reader goes to on purpose, instead of trailing every species
question. **Species notes route fishing decisions; this note answers "can I keep
it."**

> **Regulations change. The figures below were checked on the dates shown and
> may already be out of date.**
>
> Confirm current rules with **CDFW** (California) or **CONAPESCA** (Mexico)
...
limit does not apply at Catalina.

## When to open this note

Open it when someone is deciding whether to **keep** a fish: a question about
limits or legality, or a plan step that involves a take decision. Do not surface
limits on "where do I go," "what do I throw," or "is it fishable" — that is most
of what gets asked, and reciting limits into those answers is noise.

## California (CDFW)

**checked: 2026-08-18** against the *2026 California Ocean Sport Fishing
Regulations*, last updated 2026-07-17. Verify current rules before fishing.

Lengths: **TL** = total length (tip of head to longest tip of tail), **FL** =
fork length (tip of head to fork of tail), **alt** = alternate length (base of
the foremost dorsal spine to the longest tip of tail).
...
Several rows above are "10 within the 20" — they interact.

**Take or possession prohibited:** garibaldi · giant sea bass (black sea bass) ·
white shark · coho salmon · gulf and broomtail grouper · quillback, yelloweye
and bronzespotted rockfish · cowcod · all abalone. This list is not exhaustive —
check the current booklet.

**Other California rules worth knowing:**

- **Licence.** Anyone 16 or older needs a sport fishing licence — **except when
...
**checked: 2026-08-18.** Verify current rules before fishing. Mexican
sportfishing documentation is genuinely hard to source — see the provenance note
below and treat this section as a starting point, not an authority.

**There are no size limits.** The Mexican regime is bag-based.
...
regardless of age and regardless of whether they are fishing, plus a **vessel
permit**. Confirm both before crossing.

**⚠ Open item — how a mixed bag adds up.** Whether the per-species caps and the
10-fish daily total combine cleanly is **unresolved**, and no source located
answers it. Worked example (cameron): one bluefin, five yellowtail and four
rockfish is legal on **every per-species cap**, but lands somewhere between 11.5
and 14 against a 10-fish total depending on how the bluefin are counted. **Treat
the per-species caps as the operative rule and do not assume the totals add
cleanly.**

**⚠ Open items — not resolved either way:**

- **Black sea bass** — no source found placing it in any special group.
- **Gulf grouper** — some English-language sources place it in the 2-per-day
  group; the NOM-017 text does not. Gulf grouper is separately protected in
  California, so this may be a different rule bleeding across.

### How this section is sourced

Layered, because the layers change at different rates:

- **NOM-017-PESC-1994** supplies the base structure — 10 fish per angler per
  day, no more than 5 of one species, the 1-per-day marlin/sailfish/swordfish/
  shark group, and the 2-per-day tarpon/dorado/roosterfish group.
- **Bluefin is a later, separate measure (2015)**, which is why it does not
  appear in the 1995 norm: 2 per angler per day, counting as 5, with a 6-fish
  trip maximum.
- **Trip caps** are fleet-operational practice on the San Diego long-range
  boats rather than text quoted from the norm.
...
reader crossing a border should confirm rather than rely on this.
